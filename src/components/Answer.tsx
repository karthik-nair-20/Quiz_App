interface options {
  option: string,
  onClick: (answerText: string) => void,
  selectedAns: string
}

export function Answer({ option, onClick, selectedAns }: options) {
  return (
    <div className={`bg-white px-4 pb-2 rounded-full min-w-[250px] shadow-sm border hover:shadow-md focus:outline-none focus:ring-2 ${option === selectedAns ? 'bg-green-300 border-2 border-green-700' : ''} `}>
      ...
      <button className="text-lg sm:text-base font-normal mb-2 pl-4"
